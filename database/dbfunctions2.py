...
DATABASE_PATH = "E:/Project25/J_Blosmia1/database/database_wbc.db"
...
def add_count_in_reportResult(cell_count_data):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "insert into ReportResult(report_id,Basophil,Neutrophil,Eosinophil,Lymphocyte,Monocyte) values(?,?,?,?,?,?)",
            cell_count_data,
        )
        conn.commit()
        conn.close()
